...
class Album(SQLObject):
    title = StringCol(length=160, varchar=True)
    artist = ForeignKey('Artist', default=None)
...
if __name__ == '__main__':
    ...
    for album in Album.select(AND(Album.q.artistID == Artist.q.id,
    ...
    init = input("\nbusco albums de los artistas que comiencen con: ")
    query = "SELECT artist.name, album.title FROM artist, album WHERE artist.id=album.artist_id AND artist.name LIKE '%s%%'" % init
    print(query)
